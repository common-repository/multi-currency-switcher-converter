...
* @since	1.0.4
*
* @uses 	mcsc_perform_conversion  Perform the actual currency conversion
*
...
function mcsc_convert_currency( $number = '', $from = '', $to = '', $dp = '', $template = '' ) {
    ...
    if ( $number == '' ) {
        ...
        $result = 'Please add a number';

    } else {

        // Perform the conversion

        $result = mcsc_perform_conversion( $number, $from, $to, $dp );
    }
    ...
    if ( substr( $result, 0, 1 ) == '#' ) {
        if ( $suppress_errors ) {
            ...
            return '<span style="font-weight: bold; color: #f00;">' . __( 'Multi-Currency Switcher Converter Error: ', 'multi-currency-switcher-converter' ) . substr( $result, 1 ) . '</span>';
        }
	} else {
...
* @since	1.0.4
*
* @uses 	mcsc_get_options         Get the default options
* @uses     mcsc_get_rates           Get the array of exchange rates
...
function mcsc_perform_conversion( $number = '', $from = '', $to = '', $dp = '' ) {
    ...
    if ( is_array( $rates_array ) ) {
        ...
        if ( ( $from == '' ) or ( $to == '' ) ) {
            ...
            $error = 'Could not fetch API content';
